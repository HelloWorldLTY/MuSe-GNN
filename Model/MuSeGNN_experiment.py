...
for item in tissue_list_all.keys():
    ...
    class GCNEncoder_Multiinput(torch.nn.Module):
        def __init__(self, out_channels, graph_list, label_list):
            ...
            conv_dict = {}
            for i in graph_list:
                conv_dict[i.show_index] = torch_geometric.nn.Sequential('x, edge_index', [(TransformerConv(i.x.shape[1], out_channels * 4),'x, edge_index-> x'),
    ...
    class GCNEncoder_Commoninput(torch.nn.Module):
        def __init__(self, out_channels, graph_list, label_list):
            ...
            for i in tissue_specific_list:
                conv_dict_l2[i] = torch_geometric.nn.Sequential('x, edge_index', [(TransformerConv(out_channels*4, out_channels * 2),'x, edge_index -> x'),
    ...
    for seed in range(0, 10):
        ...
        loss_func = losses.NTXentLoss(temperature = args.temp)
        ...
        edge_adj_list = [torch.FloatTensor(cor_list[i].values) for i in graph_index_list]
        ...
        for epoch in range(args.epoches):
            ...
            for i in range(0,len(graph_index_list)):
                ...
                edge_adj = edge_adj_list[i].to(device)
        ...
        adata.write_h5ad(f"wslgnn_benchmark/{tissue.split('_')[1]}_umi_wslgnn_{seed}_{finalname}.h5ad")