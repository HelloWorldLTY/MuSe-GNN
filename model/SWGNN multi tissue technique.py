...
tissue_list = { 
 ...
 'H2',
    ...
    "scrna_lung":["BAL034", "A44-LNG-2-SC-45N-1"],
    "scrna_kidney":["b1", "b2"],
        "scrna_liver":["A31", "A29", ],
        "scrna_thymus":["A31"],
    "scrna_spleen":["A52", "640C",],
  "spatial_heart":['G19b18',
 'G19b19']
               }
...
for tissue in tissue_list.keys():
    for i in tissue_list[tissue]:
        ...
        graph.class_type = torch.tensor([tissue_map[tissue] for _ in range(len(graph.x))])
...
gene_dism = Discriminator(32,7).to(device)
...
optimizer_dis = torch.optim.Adam(gene_dism.parameters(), lr=1e-3)

loss_f = nn.BCELoss()
loss_m = nn.CrossEntropyLoss()

Z = np.load("graph_sim_subsample.npy")
    ...
    for i in range(0,len(graph_list)):
        ...
        if epoch%10 == 0:
            for j in range(0,len(graph_list)):
                if i==j:
                    continue
                graph_new = graph_list[j].to(device)
                
                x = graph_new.x
                train_pos_edge_index = graph_new.edge_index
                ...
                id1, id2 =  np.random.randint(0, len(z_diff), len(z_diff)//100), np.random.randint(0, len(z_new_diff), len(z_new_diff)//100)
    
                embeddings = torch.cat((z_diff[id1], z_new_diff[id2]))
                labels = torch.cat((torch.zeros(len(id1)), torch.ones(len(id2)))).to(device)
                loss_neg = NTXentLoss(temperature=0.05) #InfoNCE
...
adata.write_h5ad("/ysm-gpfs/pi/zhao/tl688/GIANT/GIANT/src/analysis/multi_folder/umi_sample_SWMGNN_infoNCE_nocommongene.h5ad")